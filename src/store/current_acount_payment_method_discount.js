...
export default {
	...
	state: {
		...
		route_string: 'cc-payment-method-discount',
		route_prefix: '',
		from_dates: false,
	...
	actions: {
		...
		delete({ commit, state }) {
			let model_name
			if (typeof state.route_string != 'undefined' && state.route_string) {
				model_name = state.route_string
			} else {
				model_name = generals.methods.routeString(state.model_name)
			}

			return axios.delete(`/api/${model_name}/${state.delete.id}`)
			.then(() => {
				commit('delete')
			})
			.catch((err) => {
