...
<script>
/*
	Listado de sugerencias de compra. Se apoya en el store purchase_suggestion
	(GET purchase-suggestion), mismo patron que sugerencias-de-stock/Listado.vue.
*/
export default {
	components: {
		FormNueva: () => import('@/components/sugerencias-de-compra/FormNueva'),
	},
	computed: {
		sugerencias() {
			return this.$store.state.purchase_suggestion.models
		},
		loading() {
			return this.$store.state.purchase_suggestion.loading
		},
		fields() {
			return [
				{
	...
	methods: {
		 ...
		 */
		refrescar() {
			this.$store.dispatch('purchase_suggestion/getModels')
		},
		/**
		 * Abre el modal de creacion (FormNueva).
		 */
		nueva_sugerencia() {
			this.$bvModal.show('sugerencia-compra-form-nueva')
		},
		/**
		 * Navega al detalle de la sugerencia clickeada.
		 */
		ver_detalle(sugerencia) {
			this.$router.push({name: 'sugerencias_compra', params: {id: '' + sugerencia.id}})
		},
		/**
