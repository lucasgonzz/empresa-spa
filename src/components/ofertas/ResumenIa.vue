<template>
	<div class="resumen-ia-ofertas">

		<!-- Escribiendose: la corrida termino y el job del resumen esta en la cola -->
		<b-card
		v-if="estado == 'pendiente'"
		class="card-modulo resumen-ia-ofertas__card m-b-15">
			<div class="j-start align-center">
				<b-spinner
				small
				class="m-r-10"></b-spinner>
				<span class="text-muted">
					Lo estamos escribiendo: en un momento vas a tener un resumen de estas ofertas.
				</span>
			</div>
		</b-card>

		<!-- Listo: el texto en criollo que escribio la IA sobre lo ya calculado -->
		<b-card
		v-else-if="estado == 'listo' && corrida.resumen_ia"
		class="card-modulo resumen-ia-ofertas__card m-b-15">
			<h6 class="resumen-ia-ofertas__titulo">
				<i class="bi bi-stars m-r-5"></i>
				Resumen
			</h6>
			<p class="resumen-ia-ofertas__texto m-b-0">
				{{ corrida.resumen_ia }}
			</p>
		</b-card>
		...
		<!--
			...
			no tener el resumen no es una falla y no se presenta como tal, y los
			porcentajes salen igual porque los calcula el motor, no la IA.
		-->
		<div
...
<script>
/*
	Resumen escrito por IA sobre la corrida de ofertas ya calculada. Molde de
	sugerencias-de-compra/ResumenIa.vue: tres estados visibles (pendiente /
	listo / error) y uno invisible (null: la cuenta no tiene IA contratada, el
	bloque no existe). En error, Reintentar vuelve a pedir el resumen al backend
	y deja el ciclo en manos del polling del Detalle.
*/
export default {
	props: {
		corrida: {
	...
	computed: {
		estado() {
			return this.corrida.resumen_ia_estado
		},
	},
	methods: {
		/**
		 ...
		 * texto cuando este listo. Si el backend rechaza (422: corrida no terminada
		 * o sin credenciales), se avisa con un toast y no se toca nada.
		 */
		reintentar() {
			let self = this
			this.enviando_reintento = true
			this.$api.post('offer-suggestion/' + this.corrida.id + '/resumen')
			.then(function() {
				self.enviando_reintento = false
				self.corrida.resumen_ia = null
				self.corrida.resumen_ia_error = null
				self.corrida.resumen_ia_estado = 'pendiente'
