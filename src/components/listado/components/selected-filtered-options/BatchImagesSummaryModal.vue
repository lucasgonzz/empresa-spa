	...
	@ok="on_confirm">
		<div class="batch-summary-content">
			...
			class="batch-summary-skipped-list">
				...
				<!-- Spinner discreto: no bloquea el resto del modal, solo esta sección de detalle. -->
...
export default {
	...
	data() {
		return {
			...
			/* Indica si la carga del detalle falló (no rompe el modal, solo oculta el detalle expandible). */
			detail_error: false,
	...
	methods: {
		...
		/**
		 * Al mostrarse el modal, si el payload trae `batch_uuid` pide el detalle por criterio.
		 * El resto del modal (contadores, listas) ya se ve con lo que trajo el evento de Pusher,
		 * sin esperar esta petición.
		 *
		 * @return {void}
		 */
		on_shown() {
