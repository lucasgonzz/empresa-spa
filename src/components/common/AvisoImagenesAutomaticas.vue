...
/**
 * Anfitrión del aviso de "Asignar imágenes automáticamente": se monta en `App.vue` y vive en
 * TODA la aplicación.
 *
 ...
 *
 * El disparador es `SearchImageAutomatica.vue`, que vive adentro del dropdown de opciones del
 * listado (`OptionsDropdown.vue`, `v-if="show"` sobre la cantidad de seleccionados). Apenas el
 * lote arranca, el flujo normal del usuario es deseleccionar o cambiar de vista, y ahí el
 ...
 * "hay un lote en vuelo en esta pestaña", el pedido del resumen y el modal. El disparador quedó
 * reducido a pedir el POST y avisar por el bus de `$root`.
 *
 * 🔴 La suscripción es BAJO DEMANDA, no permanente, y la diferencia no es de estilo:
 * `article_batch_images.{id}` es un canal del OWNER, no del usuario. Si este anfitrión se
...
const EVENTO_LOTE_INICIADO = 'imagenes-automaticas:lote-iniciado'

/** Canal público del comercio, igual que `article_embeddings.{id}` e `import_status.{id}`. */
const PREFIJO_DEL_CANAL = 'article_batch_images.'
 ...
 */
const MARCA_LOTE_EN_VUELO = 'imagenes_automaticas_lote_en_vuelo'
...
export default {
	...
	data() {
		return {
			 ...
			 * `article_batch_images.{owner_id}` es PÚBLICO y se nombra por el id del owner: dos
			 * instalaciones con el mismo owner sobre la misma app de Pusher comparten canal
			 * literalmente (medido el 28/8/2026 entre dos instancias de demo, las dos con
			 * owner 300). Sin este filtro, la pestaña acepta el primer evento que pasa, se da de
			 * baja del canal y se queda esperando para siempre el suyo: las imágenes se asignan
			 * bien y el modal no aparece nunca.
			 *
	...
	beforeDestroy() {
		...
		if (this.imagenes_echo_channel && this.Echo) {
			this.Echo.leaveChannel(this.imagenes_echo_channel)
			this.imagenes_echo_channel = null
		}
	},
	watch: {
		/**
	...
	methods: {
		/**
		 * El disparador (`SearchImageAutomatica.vue`) encoló un lote: a partir de acá esta
		...
		sincronizar_suscripcion() {
			...
			if (this.imagenes_echo_channel === canal) {
				return
			}
			...
			if (this.imagenes_echo_channel) {
				this.Echo.leaveChannel(this.imagenes_echo_channel)
			}
			this.imagenes_echo_channel = canal
			if (!canal) {
				return
			}

			/*
				Va con `.listen('.ArticleBatchImagesProcessed')`, CON el punto inicial: lo que
				viaja es un ShouldBroadcastNow con broadcastAs, no una notificación de Laravel.
		...
		/**
		 * Llegó el aviso de fin del lote.
		 *
		 * Se deja de esperar y se suelta el canal ANTES de pedir el resumen: el pedido puede
		 * tardar, y hasta que vuelva no hay ninguna razón para seguir escuchando.
		 *
